...
const GlobalLog = () => {
	...
	const [createdTickets, setCreatedTickets] = useState<
		TicketWithNames[] | undefined
	>();
	const [helpedTickets, setHelpedTickets] = useState<
		TicketWithNames[] | undefined
	>();
	const toast = useToast();
	const { refetch: fetchTicketsWithEmail } =
		trpc.ticket.getTicketsWithUserEmail.useQuery(
		...
		<Flex mt={4} ml={4} mr={4} flexDirection="column">
			...
			<Text mb={3} display="inherit">
				<Link href="/activity/global">
					<Text _hover={{ cursor: "pointer" }} fontStyle="italic">
