...
const StatsGraph = (props: StatsGraphProps) => {
  ...
  const binStatsByTime = () => {
    ...
    const filteredStatsInRange = stats.filter((s) => {
      return (
        s.resolvedAt &&
        timeRange.startTime <= s.resolvedAt &&
        s.resolvedAt < timeRange.endTime
      );
    });

    const bins: { [key: string]: TicketStats[] } = {};
    for (
      let t = timeRange.startTime;
      t <= timeRange.endTime;
      t = timeRange.type!.increment(t)
    ) {
      const start = t;
      const end = timeRange.type!.increment(start);
      bins[timeRange.type!.formatString(start)] = filteredStatsInRange.filter(
        (s) => {
  ...
  const getResolveTimeStats = (bins: { [key: string]: TicketStats[] }) => {
    const data = Object.keys(bins).map((b) => ({
      name: b,
      data: bins[b]!.filter((s) => s.createdAt && s.resolvedAt)
        .map((t) => resolveTime(t))
        .sort(),
    }));
  ...
  const getNumberOfTicketStats = (bins: { [key: string]: TicketStats[] }) => {
    return Object.keys(bins).map((b) => ({
      ...
      numberOfUnresolvedTickets: bins[b]?.filter(
        (s) => s.status === TicketStatus.CLOSED,
      ).length,
    }));
  };
      ...
      case StatType.HELP_TIME:
        return (
          <>
            <Line
              type="monotone"
              dataKey="meanHelpTime"
              name="Mean Help Time"
              stroke="#3486eb"
            />
            <Line
              type="monotone"
              dataKey="medianHelpTime"
              name="Median Help Time"
              stroke="#8884d8"
            />
          </>
        );
      case StatType.RESOLVE_TIME:
        return (
          <>
            <Line
              type="monotone"
              dataKey="meanResolveTime"
              name="Mean Resolve Time"
              stroke="#3486eb"
            />
            <Line
              type="monotone"
              dataKey="medianResolveTime"
              name="Median Resolve Time"
              stroke="#8884d8"
            />
          </>
        );
      case StatType.NUMBER_OF_TICKETS:
        return (
          <>
            ...
            <Line
              type="monotone"
              dataKey="numberOfUnresolvedTickets"
              name="Number of Unresolved Tickets"
              stroke="#8884d8"
            />
          </>
        );
  ...
  useEffect(() => {
    if (statType !== undefined && statType.value === StatType.HELP_TIME) {
      ...
      statType !== undefined &&
      statType.value === StatType.RESOLVE_TIME
    ) {
      setData(getResolveTimeStats(binStatsByTime()));
    } else if (
      statType !== undefined &&
      statType.value === StatType.NUMBER_OF_TICKETS
    ) {
    ...
    <Flex direction="column" w="100%" h="100%">
      <FormControl w="100%" mt={6} isRequired>
        <Flex direction={"row"} justifyContent={"space-between"}>
          <Select
            value={statType}
            onChange={(val) => setStatType(val ?? undefined)}
