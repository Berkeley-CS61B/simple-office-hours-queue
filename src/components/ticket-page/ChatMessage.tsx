...
const ChatMessage = ({ message, canSeeName }: ChatMessageProps) => {
  ...
  const [messageBlur, setMessageBlur] = useState<MessageBlur>(
    visibleToStudents ? "none" : "blur(5px)",
  );
  ...
  const backgrounColor = !visibleToStudents
    ? chatMessageColors.notVisibleToStudents
    : amISender
      ? chatMessageColors.visibleToStudents
